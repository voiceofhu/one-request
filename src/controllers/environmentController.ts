...
export class EnvironmentController {
  private static readonly noEnvironmentPickItem: EnvironmentPickItem = {
    ...
    description:
      "You can still use variables defined in the $shared environment",
  };
  ...
  private constructor(initEnvironment: EnvironmentPickItem) {
    ...
    this.environmentStatusEntry = new EnvironmentStatusEntry(
      initEnvironment.label,
    );
  }
  ...
  public async switchEnvironment() {
    ...
    const userEnvironments: EnvironmentPickItem[] = Object.keys(
      ...
      .map((name) => ({
        ...
        label: name,
        description:
          name === this.currentEnvironment.name ? "$(check)" : undefined,
      }));
    ...
    const item = await window.showQuickPick(itemPickList, {
      placeHolder: "Select One Request Environment",
    });
    if (!item) {
      return;
    }
